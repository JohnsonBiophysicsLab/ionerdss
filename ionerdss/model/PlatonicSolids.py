...
This module defines the `PlatonicSolidsModel` class, inheriting from the `Model` class, which is used to generate NERDSS molecule types and reactions, and corresponding files for platonic solid.
"""
...
class PlatonicSolid(Model):
    ...
    @classmethod
    def create_Solid(cls, solid_type: str, radius: float, sigma: float = None) -> Model:
        """
        Parameters:
            cls:
        ...
        # this returns the molecule information needed to generate a model class
        molecule_interfaces = []
        ...
        if solid_type == 'dode':
            if sigma == None:
                raise ValueError(
                    f"if Solid Type is {solid_type}. Sigma must be provided. Argument is currently sigma={sigma}")
            dode_reaction_parameters, dode_mol_information = dode_face_write(
                radius, sigma, create_Solid=True)
            ...
            for i in dode_mol_information.keys():
                ...
                molecule_interfaces.append(MoleculeInterface(
                    name=i, coord=Coords(x_coord, y_coord, z_coord)))

            molecule = [MoleculeType(
                name='dode', interfaces=molecule_interfaces)]
        if solid_type == 'cube':
            if sigma == None:
                raise ValueError(
                    f"if Solid Type is {solid_type}. Sigma must be provided. Argument is currently sigma={sigma}")
            cube_reaction_parameters, cube_mol_information = cube_face_write(
                radius, sigma, create_Solid=True)
            ...
            molecule = [MoleculeType(
                name='cube', interfaces=molecule_interfaces)]
        if solid_type == 'icos':
            if sigma == None:
                raise ValueError(
                    f"if Solid Type is {solid_type}. Sigma must be provided. Argument is currently sigma={sigma}")
            icos_reaction_parameters, icos_mol_information = icos_face_write(
                radius, sigma, create_Solid=True)
            ...
            molecule = [MoleculeType(
                name='icos', interfaces=molecule_interfaces)]
        if solid_type == 'octa':
            if sigma == None:
                raise ValueError(
                    f"if Solid Type is {solid_type}. Sigma must be provided. Argument is currently sigma={sigma}")
            octa_reaction_parameters, octa_mol_information = octa_face_write(
                radius, sigma, create_Solid=True)
            ...
            molecule = [MoleculeType(
                name='octa', interfaces=molecule_interfaces)]
        if solid_type == 'tetr':
            if sigma == None:
                raise ValueError(
                    f"if Solid Type is {solid_type}. Sigma must be provided. Argument is currently sigma={sigma}")
            tetr_reaction_parameters, tetr_mol_information = tetr_face_write(
                radius, sigma, create_Solid=True)
            ...
            molecule = [MoleculeType(
                name='tetr', interfaces=molecule_interfaces)]
