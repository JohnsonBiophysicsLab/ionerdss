...
def test_compute_size_distribution():
    ...
    # Prob size 1 = 10/17
    assert np.isclose(df.loc[0, 'probability'], 10/17)
    assert np.isclose(df.loc[1, 'probability'], 5/17)
    assert np.isclose(df.loc[2, 'probability'], 2/17)
...
def test_compute_free_energy():
    ...
    assert np.isclose(df_fe.loc[0, 'free_energy'], g1)
    ...
    if df_fe['free_energy'].min() == 0.0:
        assert np.isclose(df_fe.loc[0, 'free_energy'], expected_g1_shifted)
